...
const Export: React.FC<ExportProps> = ({ tutorials, visible, setVisible }) => {
    ...
    return (
        <div className={visible ? "" : "hidden"} id="export-container" onClick={() => setVisible(false)}>
            <div className="rounded-item" id="export-window" onClick={event => event.stopPropagation()}>
                <pre>
                    <code className="language-json" id="export-code" >{exportCode}</code>
                </pre>
            </div>
        </div>
    );
