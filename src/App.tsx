...
function App() {
  ...
  const [selectedControl, setSelectedControl] = React.useState<Control>(Control.Empty);
  const [selectedFilePath, setSelectedFilePath] = React.useState<string>("");
  const [highlightedComponents, setHighlightedComponents] = React.useState<Component[]>([]);
  ...
  const updateSelectedTutorial = React.useCallback((updatedTutorial: Tutorial) => {
    if (!selectedTutorial) return;
    const updatedTutorials = [...tutorials];
    updatedTutorials[tutorials.indexOf(selectedTutorial)] = updatedTutorial;
    setTutorials(updatedTutorials);
    setSelectedTutorial(updatedTutorial);
  }, [tutorials, selectedTutorial])

  const updateSelectedTask = React.useCallback((updatedTask: Task) => {
    if (!selectedTutorial || !selectedTask) return;
    const updatedTasks = [...selectedTutorial.tasks];
    updatedTasks[selectedTutorial.tasks.indexOf(selectedTask)] = updatedTask;
